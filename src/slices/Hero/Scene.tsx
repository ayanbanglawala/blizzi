...
const Scene = (props: Props) => {
  ...
  useGSAP(()=>{
    ...
    // gsap.set(can1Ref.current.rotation, {y:Math.PI})
    // gsap.set(can2Ref.current.rotation, {y:Math.PI})
    // gsap.set(can3Ref.current.rotation, {y:Math.PI})
    // gsap.set(can4Ref.current.rotation, {y:Math.PI})
    // gsap.set(can5Ref.current.rotation, {y:Math.PI})
  ...
  return (
    <group ref={groupRef}>
      ...
      <FloatingCan ref={can4Ref} flavour="lemonLime" floatSpeed={FLOAT_SPEED} />
      <FloatingCan
        ref={can5Ref}
